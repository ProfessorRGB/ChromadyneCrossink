...
Chromadyne DRU caches data aggressively on the SD card to minimize RAM use. The ESP32-C3 has about 380 KB of usable RAM, so rebuilding every book structure in memory on every open would be too expensive.
...
├── crossink-settings.json  # Chromadyne DRU device settings
...
Cache folders are path-based. Moving a book file can create a new cache directory, so the moved copy may start with fresh reading progress unless the firmware migrates the cache for that move. Chromadyne DRU migrates cache and bookmark data for the built-in move-to-Read flow and related file-browser move actions.

EPUB reader font, page layout, styling, and reading-aid settings normally come from the global Reader settings. If those settings are changed from inside an EPUB, Chromadyne DRU stores a per-book override in that book's `reader_settings.bin`; books without that override continue to follow the global defaults. EPUB render mode is also stored per book so a problematic title can be switched to Balanced or Light rendering from the File Browser or Recent Books long-press menus before opening it.
...
in-reader highlight, the clipping list, and jump-back behavior. Chromadyne DRU also
...
Cache data is cleared by supported Chromadyne DRU delete/move flows. If you remove or rename books outside Chromadyne DRU by editing the SD card directly, old cache folders may remain until you clear reading cache.
