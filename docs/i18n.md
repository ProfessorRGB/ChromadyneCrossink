...
Chromadyne DRU keeps user-facing UI text in per-language YAML files and generates the C++ lookup tables used by the firmware.
...
Current firmware falls back to `/.crosspoint/settings.json` when the namespaced Chromadyne DRU settings file is not present, then saves the loaded settings to `crossink-settings.json`.
