...
struct ReaderSettingsBin {
    ...
    u16 autoPageTurnSeconds;
    u8 renderMode; // 0 = Chromadyne DRU Default, 1 = Balanced, 2 = Light
...
saved clipping is also what Chromadyne DRU renders as an in-reader highlight; there is
...
Chromadyne DRU uses the stored spine/page/paragraph fields as anchors, then searches
...
When Chromadyne DRU moves an EPUB through its built-in move-to-Read flow, it rewrites
the clipping file under the new path-derived name and removes the old one. If a
book is renamed or moved outside Chromadyne DRU, the path hash changes, so the old
...
When `stats_v5.bin` is missing, Chromadyne DRU can read the previous versioned stats
