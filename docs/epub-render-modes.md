...
Chromadyne DRU normally tries to render EPUBs with the full Chromadyne DRU renderer. Some
...
asking Chromadyne DRU to simplify the layout work for that book.
...
| --- | --- | --- | --- |
| Chromadyne DRU Default | Most books | Full Chromadyne DRU styling, detailed CSS, table layout, image sizing, publisher spacing, Bionic Reading, Guide Dots | Nothing by default |
| Balanced | Books where Chromadyne DRU has fallen back automatically, or books you want to start in a lighter mode | Publisher spacing, image sizing, decorations, Bionic Reading, Guide Dots | Complex CSS lookups and table layout |
...
Most users can leave books on **Chromadyne DRU Default**. If a book runs out of memory
while building a section, Chromadyne DRU will automatically try **Balanced** and then
**Light**. Manual mode changes are mainly for choosing a lighter mode before
opening a known-problem book, or for changing the saved mode after Chromadyne DRU has
already fallen back.

## Chromadyne DRU Default

Chromadyne DRU Default is the normal renderer.

It tries to preserve the book's layout and publisher styling as much as
Chromadyne DRU supports, including:
...
Use Balanced when Chromadyne DRU has already fallen back to it, or when you want a
...
Use Light when Chromadyne DRU has already fallen back to it, or when you want a
known-problem book to start in the safest mode immediately. Light may look less
like the publisher's original layout, but it gives Chromadyne DRU the best chance of
...
If Chromadyne DRU cannot build a section in Light because memory is still too tight,
...
When Safe Mode succeeds, Chromadyne DRU saves those settings for that book. The reader
briefly shows **Safe Mode** over the current page. Your other books keep their
own reader settings.

## Automatic Fallback

If a book starts in Chromadyne DRU Default and a section runs out of memory while
building, Chromadyne DRU automatically retries in this order:

1. Chromadyne DRU Default
2. Balanced
3. Light
4. Safe Mode

If the book starts in Balanced, Chromadyne DRU can fall back to Light and then Safe
Mode. If the book starts in Light, Chromadyne DRU can still try Safe Mode.

When a fallback succeeds, Chromadyne DRU saves that successful mode for the book. The
...
Chromadyne DRU already falls back automatically, but it is useful when you know a book
